...
export class AuthService {
  ...
  async signinLocal(dto: AuthDto) {
    const user = await this.prisma.college_staff.findFirst({
      where: {
        mail: dto.email,
      },
    });
  ...
  async logout(userId: number): Promise<boolean> {
    await this.prisma.college_staff.update({
      where: {
        id: userId,
      },
      data: {
  ...
  async getTokens(userId: number): Promise<{
    ...
    const user = await this.prisma.college_staff.findUnique({
      where: {
        id: userId,
      },
    });
    ...
    if (user.role === 'hod') {
      ...
      custom_claims = {
        'x-hasura-class-id': `${user.class_id}`,
        'x-hasura-department-id': `${user.department_id}`,
      };
    } else if (user.role === 'warden') {
      custom_claims = {
        'x-hasura-hostel-id': `${user.hostel_id}`,
      };
    }
    const jwtPayload = {
      sub: `${user.id}`,
  ...
  async refreshTokensWithFirebase(token: string): Promise<string> {
    try {
      ...
      const studentData = await this.prisma.students.findFirst({
        where: {
          mail: tokenResult.email,
        },
      });
      if (!studentData) {
        throw new Error('truspass user');
      }
      const jwtPayload = {
        type: 'access_token',
        sub: tokenResult.uid,
        ...
        'https://hasura.io/jwt/claims': {
          ...
          'x-hasura-user-id': `${studentData.id}`,
          'x-hasura-user-email': tokenResult.email,
          'x-hasura-hostel-id': `${studentData.hostel_id}`,
          'x-hasura-department-id': `${studentData.department_id}`,
          'x-hasura-class-id': `${studentData.class_id}`,
        },
      };
